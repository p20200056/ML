...
for file in files:
    ...
    for m in range(epoch):
        ...
        for l in range(y_test_5.size):        
            checker = ((25*y_pred_xgbm[l])/10) + ((10*y_pred_rf[l])/10) + ((35*y_pred_svm[l])/10)
            if (checker >= 3.5):
                 accuracy_sum[l] = 1     
            else :
                 accuracy_sum[l] = 0
        ...
        Latency_list[m]=(time_ens_e-time_ens_s)+time_xgbm+time_rf+time_svm            
...
dict = {'accuracy':accuracy_list,'tn':tn_list,'fp':fp_list,'fn':fn_list,'tp':tp_list,'Latency':Latency_list,'Sensitivity':sensitivity_list,'Specificity':specificity_list}
df2 = pd.DataFrame(dict)
df2.to_csv('VWEC_Top3_cosupi_80-20.csv') 
